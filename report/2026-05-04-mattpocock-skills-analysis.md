...
>
> 目的：评估两个仓库中是否有可借鉴的 agent 角色定义和 skill 方法论，用于 agent-discussion 的 preset 角色池设计（见 `docs/plans/2026-05-04-agent-preset-design.md`）。
...
**借鉴建议**：直接把 meeting-notes-and-actions 的输出结构作为 Synthesizer 的模板基线，补上 agent-discussion 特有的"核心分歧与演进"和"主持人倾向"字段即可。
...
**核心结论**：这是对 agent-discussion preset 设计**借鉴价值最高**的仓库——既有可直接参考的 agent 角色结构，又有可改造为角色指令的 skill 方法论，还有独树一帜的"反合理化表"（anti-rationalization）设计模式。

## 9. Agent 角色文件分析（可直接借鉴）

这是三个仓库中唯一包含预定义 agent 角色的。三个角色均为"审查型"（reviewer），而非"讨论参与者"型，但其角色定义结构——领域视角、评估维度、输出格式、行为规则——可以直接用作 agent-discussion 角色指令的模板。
...
这个模式可以直接用于 agent-discussion 的 scope 约束——让讨论参与者知道"哪些借口是不可接受的"。
...
三个仓库都对 agent-discussion 的 preset 设计有贡献，但贡献类型不同：
...
两个仓库都不包含可直接引用的 agent 角色定义——它们本质上是**工作流自动化工具**和**工程方法论 skill**，不是"人设指令集"。但以下几个 skill 的内部方法论对 agent-discussion 的 preset 角色设计具有高价值借鉴意义：
